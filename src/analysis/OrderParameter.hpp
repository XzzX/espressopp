    ...
    void biggest_cluster()
    {
        ...
        if (getSystem()->comm->rank() == 0)
        {
            ...
            */
            ...
            int count=0;
            for (vector<int>::iterator it = clusters[max_pos].begin() ; it !=
            clusters[max_pos].end(); ++it){ cout<<count<<"  BIGGEST CL  cpu: "<<
            getSystem()->comm->rank() <<  " id: "<< *it << endl; count++;
            }
            */
